...
class UploadPurchaseExcel extends StatefulWidget {
  const UploadPurchaseExcel({super.key});

  @override
  State<UploadPurchaseExcel> createState() => _UploadPurchaseExcelState();
}

class _UploadPurchaseExcelState extends State<UploadPurchaseExcel> {
  String fileName = "";
  int? size;
  ...
  void uploadToServer() async {
    try {
      ...
      setState(() {
        loading = false;
        unknownProducts = _unknownProducts;
      });
      Get.back(result: {'json': json, 'unknownProducts': _unknownProducts});
    } catch (e) {
      setState(() {
