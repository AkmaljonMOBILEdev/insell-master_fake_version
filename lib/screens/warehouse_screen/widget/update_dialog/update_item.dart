...
class UpdateItem extends StatefulWidget {
  ...
  final ProductIncomeDto product;
  final int index;
...
class _UpdateItemState extends State<UpdateItem> {
  ProductIncomeDto get product => widget.product;
    ...
    return AppTableItems(
      ...
      items: [
        ...
        AppTableItemStruct(
          innerWidget: Center(
            child: AppReadAndWriteWidget(
              value: formatNumber(product.suggestedPrice),
              setter: (String val) {
                setState(() {
                  product.suggestedPrice = double.tryParse(val) ?? 0;
                });
              },
            ),
          ),
        ...
        AppTableItemStruct(
          innerWidget: Center(
            child: Text(
              "${formatNumber(((product.suggestedPrice ?? 0) - product.price) / product.price * 100)} %",
              style: TextStyle(color: AppColors.appColorWhite, fontSize: 14),
            ),
          ),
        ),
