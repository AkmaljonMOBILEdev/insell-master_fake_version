...
class PaySendToAnotherCashDialog extends StatefulWidget {
  const PaySendToAnotherCashDialog({Key? key}) : super(key: key);

  @override
  State<PaySendToAnotherCashDialog> createState() => _PaySendToAnotherCashDialogState();
}

class _PaySendToAnotherCashDialogState extends State<PaySendToAnotherCashDialog> {
    ...
    return AppDialog(
      ...
      title: Column(
        children: [
          ...
          Text('Boshqa kassaga chiqarish', style: TextStyle(color: AppColors.appColorWhite, fontSize: 20)),
        ],
      ),
      content: SizedBox(
        height: 300,
  ...
  void createPayToAnotherCash() async {
    try {
      var req = {
        "invoice": {
      ...
      response = await HttpServices.post("/consumption/create/pos-consumption", req);
      Get.back();
      print(response.body);
