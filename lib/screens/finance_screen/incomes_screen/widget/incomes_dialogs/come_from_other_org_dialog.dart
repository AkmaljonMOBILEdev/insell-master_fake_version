...
class ComeFromOtherOrgDialog extends StatefulWidget {
  const ComeFromOtherOrgDialog({Key? key}) : super(key: key);

  @override
  State<ComeFromOtherOrgDialog> createState() => _ComeFromOtherOrgDialogState();
}

class _ComeFromOtherOrgDialogState extends State<ComeFromOtherOrgDialog> {
    ...
    return AppDialog(
      ...
      title: Column(
        children: [
          ...
          Text('Boshqa organizatsiyadan kirim', style: TextStyle(color: AppColors.appColorWhite, fontSize: 20)),
        ],
      ),
      content: SizedBox(
        height: 300,
  ...
  void onIncomeCreate() async {
    try {
      if (_formValidation.currentState!.validate()) {
        ...
        var res = await HttpServices.post('/income/create/organization-income', {
          "invoice": {
