...
class ComeFromCounterpartyDialog extends StatefulWidget {
  const ComeFromCounterpartyDialog({Key? key}) : super(key: key);

  @override
  State<ComeFromCounterpartyDialog> createState() => _ComeFromCounterpartyDialogState();
}

class _ComeFromCounterpartyDialogState extends State<ComeFromCounterpartyDialog> {
  ...
  void getCounterPartyList() async {
    var response = await HttpServices.get("/counterparty/partner/all");
  ...
  void onIncomeCreate() async {
    try {
      if (_formValidation.currentState!.validate()) {
        ...
        var res = await HttpServices.post('/income/create/counterparty-loan-income', {
          "invoice": {
    ...
    return AppDialog(
      ...
      title: Column(
        children: [
          ...
          Text('Kontragentdan kirim', style: TextStyle(color: AppColors.appColorWhite, fontSize: 20)),
        ],
      ),
      content: SizedBox(
        height: 300,
