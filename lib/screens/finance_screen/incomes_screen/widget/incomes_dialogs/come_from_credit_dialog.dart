...
class ComeFromCreditDialog extends StatefulWidget {
  const ComeFromCreditDialog({Key? key}) : super(key: key);

  @override
  State<ComeFromCreditDialog> createState() => _ComeFromCreditDialogState();
}

class _ComeFromCreditDialogState extends State<ComeFromCreditDialog> {
    ...
    try {
      if (_formValidation.currentState!.validate()) {
        if (_counterparty == null) {
          throw Exception('Bankni tanlang');
        }
        ...
        var res = await HttpServices.post('/income/create/credit-loan-income', req);
