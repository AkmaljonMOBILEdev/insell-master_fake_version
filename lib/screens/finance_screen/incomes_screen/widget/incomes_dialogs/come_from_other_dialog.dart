...
class ComeFromOtherDialog extends StatefulWidget {
  const ComeFromOtherDialog({Key? key}) : super(key: key);

  @override
  State<ComeFromOtherDialog> createState() => _ComeFromOtherDialogState();
}

class _ComeFromOtherDialogState extends State<ComeFromOtherDialog> {
  ...
  void onIncomeCreate() async {
    try {
      if (_formValidation.currentState!.validate()) {
        ...
        var res = await HttpServices.post('/income/create/other-income', {
          "invoice": {
    ...
    return AppDialog(
      ...
      title: Column(
        children: [
          ...
          Text('Boshqa kirimlar', style: TextStyle(color: AppColors.appColorWhite, fontSize: 20)),
        ],
      ),
      content: SizedBox(
        height: 300,
