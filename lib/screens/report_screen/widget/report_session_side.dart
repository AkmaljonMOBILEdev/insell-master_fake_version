...
class ReportTradesByPosSession extends StatefulWidget {
  const ReportTradesByPosSession({super.key});

  @override
  State<ReportTradesByPosSession> createState() => _ReportTradesByPosSessionState();
}

class _ReportTradesByPosSessionState extends State<ReportTradesByPosSession> {
  ...
  List<PosSessionTradeStruct> posSessions = [];
  String? errorMessage;
  int activeIndex = 0;
  ...
  void getData() async {
    try {
      ...
      List<PosSessionTradeStruct> tempSessionTrades = [];
      for (var trade in trades) {
        bool isExist = false;

        for (var posSession in tempSessionTrades) {
          if (posSession.posSession?.id == trade.posSession?.id) {
            posSession.trades.add(trade);
            isExist = true;
            break;
          }
        }
        if (!isExist) {
          tempSessionTrades.add(PosSessionTradeStruct(posSession: trade.posSession, trades: [trade]));
        }
      }
      posSessions = tempSessionTrades;
        ...
        children: [
          ...
          Expanded(
            ...
            child: ReportHeader(
              ...
              headers: [
                HeaderStruct(title: 'Sessiya', index: 0, activeIndex: activeIndex, desc: descending, sort: () {}, flex: 2),
                ...
                : errorMessage != null
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          return TradeItem(trade: posSessions[index]);
                        },
...
class TradeItem extends StatefulWidget {
  ...
  final PosSessionTradeStruct trade;
...
class _TradeItemState extends State<TradeItem> {
  PosSessionTradeStruct get trade => widget.trade;
          ...
          Expanded(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${formatDateTime(trade.posSession?.startTime)} - ${formatDateTime(trade.posSession?.endTime)}',
...
class PosSessionTradeStruct {
  PosSessionDto? posSession;
  List<TradeStruct> trades = [];

  PosSessionTradeStruct({this.posSession, required this.trades});
}
